...
$sql = "UPDATE `is`.`prod_info` SET `prodName` = '$prodName', `prodMan` = '$prodMan', `prodDesc` = '$bigDesc', `prodPrice` = '$prodPrice', `prodInv` = '$inStock', `prodMinDesc` = '$smallDesc' WHERE `prod_info`.`prodID` = '$prodID'";
if (!mysqli_query($con, $sql)){
$result= "Error: ".mysqli_error($con);
}
else {$result= "Οι αλλαγές σας εφαρμόστηκαν με επιτυχία!";}
}
?>
...
<div class="container" id="maincontent" style="margin-top:50px">
    <h3>Αλλαγή προϊόντος</h3>
  <p><?php echo "$result"; ?></p>
  <button  type="button" onclick="$('#maincontent').load('change.php')" class="btn btn-default">Αλλαγή ξανά</button>
</div>
